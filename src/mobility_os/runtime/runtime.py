...
class MobilityRuntime:
    def __init__(
        self,
        scenario: str = "corridor_congestion",
        seed: int = 42,
        hotspots_csv: Optional[str] = None,
        persist_root: str | Path | None = "runs",
        ...
        self.run_store = RunStore(persist_root) if persist_root else None
        ...
        self._build_twins()
        if self.run_store and self.auto_persist:
            self.run_store.create_run({
                "scenario": self.scenario,
                "seed": self.seed,
                "title": self.scenarios[self.scenario].title,
                "mode": self.scenarios[self.scenario].mode,
            }, run_id=self.run_id)
    ...
    def step(self, dt_h: float = 5.0 / 60.0) -> MobilityExecRecord:
        ...
        if self.run_store and self.auto_persist:
            self.run_store.append_record(self.run_id, record.to_dict())
        return record
    ...
    def reset(self) -> None:
        self.__init__(scenario=self.scenario, seed=self.seed, hotspots_csv=self.hotspots_csv, persist_root=self.persist_root)
